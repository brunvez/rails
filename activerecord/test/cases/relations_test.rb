...
class RelationTest < ActiveRecord::TestCase
  ...
  def test_wharel
    posts = Post.wharel { (author_id != 1).and(id > 2) }
    assert_equal 6, posts.to_a.size
  end

  def test_wharel_can_chain
    posts = Post.wharel { author_id >= 1 }.wharel { id <= 4 }
  ...
  def tests_finding_with_block_on_joined_table
    firms = DependentFirm.joins(:account).where { (name == "RailsCore").and(accounts.credit_limit.in(55..60)) }.to_a
    assert_equal 1, firms.size
    assert_equal companies(:rails_core), firms.first
  end
